...
for (const [i, s] of songs.entries()) {
  ...
  if (s.year != null && !/^-?\d{1,4}$/.test(String(s.year))) errors.push(`${who}: year "${s.year}" must be a 1-4 digit number, optionally negative for BCE (e.g. -400 for 400 BCE, 121 for a classical text, 1947 for modern)`)
